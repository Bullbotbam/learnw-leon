...
let bulbasaurCanndies = 16 - 5;
let caterpieCanndies = 7 - 1;
let weedleCanndies = 7 - 1;
let totalCandies = bulbasaurCanndies + caterpieCanndies + weedleCanndies;
console.log(totalCandies);

//Conditionals And Functions
//You have a charmander in your party. Charmander can only battle if the temperature is above 0 degrees celcius. Create one function that converts a Fahrenheit value to Celcius and another fuction that tells you wheither or not charmander can battle

function farToCelcius(temperature) {
	let convertedToCel = ((temperature - 32) * 5) / 9;
	return convertedToCel;
}

function canCharmanderBattle(currentTemp) {
	let converted = farToCelcius(currentTemp);
	if (converted > 0) {
...
undergroundPokemonLeague(['balbasaur', 'caterpie', 'weedle']);
